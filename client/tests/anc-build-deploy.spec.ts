 ...
 *   4. ANC schedule        — one task, 8 LMP-anchored events (12..40 wk), opens the visit form
 ...
 *   $env:DEMO=1 ; $env:LIVE_DEPLOY=1 ; pnpm --filter @cht-ui/client exec playwright test anc-build-deploy.spec.ts --headed
...
test('anc-build — hierarchy → forms → pregnancy forms → 8-visit ANC schedule', async ({
  page,
  request,
}) => {
  ...
  // Grow to 8 event cards (starts with 1). Button text is content-matched
  ...
  // tasks.js carries the 8-visit schedule anchored on LMP.
