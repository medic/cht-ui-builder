...
|---|---|---|
| **State** | ATTEMPTED | Built by the dev — **not** yet QA-verified |
| | PART | Partially built — the gap is named |
| | TODO | Not started |
| | OUT | Explicitly out of MVP scope per the design doc |
| **QA** | ☑ | QA has verified it — on a real config or via an automated spec |
| | ☐ | Awaiting QA sign-off — **tick this** |
